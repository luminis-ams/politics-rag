 ...
 */
public class CustomWeaviateEmbeddingStore implements EmbeddingStore<TextSegment> {
     ...
     */
    @Builder
    public CustomWeaviateEmbeddingStore(
            String apiKey,
            String scheme,
            String host,
            Integer port,
    ...
    public List<EmbeddingMatch<TextSegment>> findRelevant(
        ...
        return resItems.stream().map(CustomWeaviateEmbeddingStore::toEmbeddingMatch).collect(toList());
    }
    ...
    private List<String> addAll(List<String> ids, List<Embedding> embeddings, List<TextSegment> embedded) {
        ...
        for (int i = 0; i < embeddings.size(); i++) {
            ...
            resIds.add(id);
            objects.add(buildObject(id, embeddings.get(i), embedded != null ? embedded.get(i) :
                    new TextSegment("", Metadata.from(emptyMap()))));
        }
    ...
    private WeaviateObject buildObject(String id, Embedding embedding, TextSegment embedded) {
        Map<String, Object> props = new HashMap<>();
        props.put(METADATA_TEXT_SEGMENT, embedded.text());
        props.putAll(embedded.metadata().asMap());
